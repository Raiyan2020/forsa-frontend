...
*(Backend's reply to `SCAN_PERMISSIONS_BULK_UPDATE_CLARIFICATION.md`, reproduced verbatim.)*
...
- [x] Grant from the Add Permission modal succeeds with the **unchanged** payload — verified: `handleRegisterSelectedVolunteers` sends exactly this shape
- [x] Revoke via the trash icon succeeds — verified: `handleDeleteVolunteer` sends `user_ids: [id]` + `is_allowed: false`
- [x] Event-scoped permissions work with `event_id` — already sent when the page was opened from an event
- [x] Read `is_allowed` from `data[]` rather than assuming what you sent — response now typed as `ApiResponse<ScanPermissionBulkUpdateEntry[]>` in `services/attendance.ts`; the table renders from the `list` refetch, never from assumed request values
- [x] Handle `400` (missing scope) via the `msg` fallback, not `validation_errors` — `getApiErrorMessages()` falls back to `msg` when `validation_errors` is empty
- [x] Refetch `list` after a revoke (revoked rows drop out — see §6) — `refetchRegistrations()` runs in both success paths
