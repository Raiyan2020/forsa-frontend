...
function EmailVerificationPageComponent({ email: propEmail, otp_type: propOtpType, onClose, onShowLogin, onShowResetPassword, isModal }: EmailVerificationProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { t } = useTranslation();
  const selectedLanguage = useLanguageStore((s) => s.language);

  // Get email and otp_type from either props or query parameters
  const email = propEmail || searchParams.get("email") || "";
  const otp_type = propOtpType || searchParams.get("otp_type") || "";
