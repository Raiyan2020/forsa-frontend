...
export default function UnregisterLearnServeConfirmationModal({
  opportunityId,
  onClose,
  refetch,
}: UnregisterLearnServeConfirmationModalProps) {
  const { t } = useTranslation();
  const selectedLanguage = useLanguageStore((s) => s.language);

  const unregisterMutation = useMutation({
    mutationFn: unregisterFromLearnServeOpportunity,
  });
  ...
  return (
    <div className="md:w-[100%] rounded-lg bg-white filtermodal">
      <p className="text-lg text-center pb-10">
        {t("COMMON.LEARNSERVE.UNREGISTER_CONFIRMATION_MESSAGE")}
      </p>
