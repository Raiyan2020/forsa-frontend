...
interface TitleProps {
  text: string;
  variant?: "default" | "white" | "green" | "orange" | "blue" | "cyan"; // Text color variants
  className?: string; // Additional custom classes
  hasMargin?: boolean; // Control margin dynamically
  hasPadding?: boolean; // Control padding dynamically
}
...
const Title: React.FC<TitleProps> = ({
  ...
  variant = "default",
  className = "",
  hasMargin = true, // Enable margin by default
  hasPadding = false, // Disable padding by default
}) => {
  ...
  const paddingClasses = hasPadding ? "px-4 py-2" : ""; // Add padding only when required
